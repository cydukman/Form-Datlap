...
import { Pen, Eraser, RotateCcw, Upload, Image as ImageIcon, FileText, Check } from 'lucide-react';
...
export const SketchCanvas: React.FC<SketchCanvasProps> = ({
  ...
  return (
    <div className="bg-slate-50 rounded border border-slate-300 p-3 h-full flex flex-col">
      ...
      <div className="flex flex-wrap justify-between items-center pb-2 mb-2 border-b border-slate-200 gap-2">
        <label className="text-[11px] font-bold text-slate-700 uppercase">
          Denah Lokasi & Titik Pengambilan Contoh Uji:
        </label>
        <div className="flex items-center gap-1 bg-white p-0.5 rounded border border-slate-200 text-xs">
          <button
            ...
            className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 transition-colors ${
              denahType === 'sketch' ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
          ...
          <button
            ...
            className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 transition-colors ${
              denahType === 'upload' ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            <Upload className="w-3 h-3" />
            <span>Upload Foto/Denah</span>
          </button>
          <button
            type="button"
            onClick={() => onChangeType('text')}
            className={`px-2 py-0.5 rounded text-[11px] font-medium flex items-center gap-1 transition-colors ${
              denahType === 'text' ? 'bg-emerald-600 text-white' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
